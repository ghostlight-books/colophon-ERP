...
type AdminStore = { id: string; slug: string; storeName: string; ownerEmail: string; ledgerBalance: number; subscriptionStatus: string; hasStoreAccess: boolean; hasLibraryAccess: boolean; createdAt: string };
...
function AdminPage(): JSX.Element {
  ...
  const [newStoreDraft, setNewStoreDraft] = useState({ storeName: "", storeSlug: "", email: "", displayName: "", password: "", tier: "STORE" as "STORE" | "LIBRARY_ONLY" });
  const [creatingStore, setCreatingStore] = useState(false);
  const [lastCreatedCredentials, setLastCreatedCredentials] = useState<{ storeName: string; email: string; password: string } | null>(null);
  ...
  async function createBetaStore(): Promise<void> {
    const { storeName, storeSlug, email, displayName, password, tier } = newStoreDraft;
    ...
    try {
      ...
      await adminPost("/admin/stores/update-access", { storeId: payload.store.id, tier });
      ...
      setLastCreatedCredentials({ storeName: payload.store.storeName, email: email.trim(), password });
      setNewStoreDraft({ storeName: "", storeSlug: "", email: "", displayName: "", password: "", tier: "STORE" });
      setActionMessage(`Beta store "${payload.store.storeName}" created on the Trial plan (${tier === "LIBRARY_ONLY" ? "Library only" : "Store + Library"}).`);
    } catch (error) {
