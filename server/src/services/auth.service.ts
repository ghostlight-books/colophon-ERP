...
export async function signIn(email: string, password: string): Promise<{
  ...
  const user = await prisma.user.findUnique({
    ...
    include: { memberships: { orderBy: { createdAt: "asc" }, include: { store: true } } },
  });
  ...
  return {
    ...
    hasStoreAccess: membership?.store.hasStoreAccess ?? true,
    hasLibraryAccess: membership?.store.hasLibraryAccess ?? true,
  };
}
